...
class AuthorizationVoided implements WebhookHandler
{
    /**
    ...
    public function invoke(Webhook $webhook)
    {
        try {
            ...
            $orderStatusModel = $this->modelManager->getRepository(Status::class)->find(PaymentStatus::PAYMENT_STATUS_CANCELLED);

            if ($orderRepository === null) {
                $this->logger->error('[AutorizationVoided-Webhook] Could not find associated order with the temporaryID ' . $webhook->getResource()['parent_payment'], ['webhook' => $webhook->toArray()]);
            ...
            $this->logger->error('[AutorizationVoided-Webhook] Could not update entity', ['message' => $ex->getMessage(), 'stacktrace' => $ex->getTrace()]);
        }
